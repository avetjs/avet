...
module.exports = function(options) {
  ...
  options.framework = utils.getFrameworkPath({
    ...
    // compatible customAvet only when call startCluster directly without framework
    framework: options.framework || options.customAvet,
  });

  const avet = require(options.framework);
  assert(avet.Application, `should define Application in ${options.framework}`);
  assert(avet.Agent, `should define Agent in ${options.framework}`);

  if (options.https) {
    assert(
      options.key && fs.existsSync(options.key),
      'options.key should exists'
    );
    assert(
      options.cert && fs.existsSync(options.cert),
      'options.cert should exists'
    );
  }
