...
module.exports = class Command {
  constructor(options) {
    options = options || {};
    this.name = options.name || 'avet-init';
    this.configName = options.configName || 'avet-init-config';
  ...
  * run(cwd, args) {
   ...
   *
   * @param {Object} mapping - boilerplate config mapping, `{ simple: { "name": "simple", "package": "avet-boilerplate-simple", "description": "Simple avet app boilerplate" } }`
  ...
  * askForVariable(targetDir, templateDir) {
    ...
    try {
      ...
      if (questions.name && !questions.name.default) {
        questions.name.default = path.basename(targetDir).replace(/^avet-/, '');
      }
    } catch (err) {
  ...
  * processFiles(targetDir, templateDir) {
  ...
  getParser() {
    return yargs
      .usage('init avet project from boilerplate.\nUsage: $0 [dir] --type=simple')
   ...
   * @param {String} key - short name, support `tnpm / china / npm / npmrc`, default to read from .npmrc
  ...
  * getTemplateDir() {
    let templateDir;
    // when use `avet-init --template=PATH`
  ...
  /**
   * fetch boilerplate mapping from `avet-init-config`
   * @param {String} [pkgName] - config package name, default to `this.configName`
   * @return {Object} boilerplate config mapping, `{ simple: { "name": "simple", "package": "avet-boilerplate-simple", "description": "Simple avet app boilerplate" } }`
   */
  * fetchBoilerplateMapping(pkgName) {
  ...
  printUsage() {
    this.log(`usage:
      - cd ${this.targetDir}
      - tnpm install
      - tnpm start / tnpm run dev / tnpm test
    `);
  }
  ...
  * downloadBoilerplate(pkgName) {
    ...
    const saveDir = path.join(os.tmpdir(), 'avet-init-boilerplate');
