...
export default function dynamicComponent(p, o) {
  ...
  if (p instanceof SameLoopPromise) {
    ...
    if (!p.modules || !p.render) {
      const errorMessage =
        'Options to `avet/dynamic` should contains `modules` and `render` fields.';
      throw new Error(errorMessage);
    }

    if (o) {
      const errorMessage =
        'Include options in the first argument which contains `modules` and `render` fields.';
      throw new Error(errorMessage);
    }
  ...
  return class DynamicComponent extends React.Component {
    constructor(...args) {
      ...
      this.LoadingComponent = options.loading
        ? options.loading
        : () => <p>loading...</p>;
...
export class SameLoopPromise {
  ...
  runIfNeeded() {
    ...
    this.ran = true;
    this.cb(result => this.setResult(result), error => this.setError(error));
  }
}
