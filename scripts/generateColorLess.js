...
less.render.call(less, content, {
  ...
  const bezierEasing = fs.readFileSync(path.join(antd, 'style/color/bezierEasing.less')).toString();
  const tinyColor = fs.readFileSync(path.join(antd, 'style/color/tinyColor.less')).toString();
  const colorPalette = fs.readFileSync(path.join(antd, 'style/color/colorPalette.less'))
    .toString()
