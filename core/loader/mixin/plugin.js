...
import { sequencify, loadFile } from '../utils';

const debug = require('debug')('avet-loader:plugin');
...
export default {
  ...
  loadPlugin() {
    ...
    const avetPluginConfigPaths = this.avetPaths.map(avetPath => path.join(avetPath, 'config/plugin.default.js'));
  ...
  readPluginConfigs(configPaths) {
    ...
    if (this.env !== 'default') {
      ...
      for (let i = 0, l = configPaths.length; i < l; i++) {
        ...
        configPaths.push(configPath.replace(/plugin\.default\.js$/, `plugin.${this.env}.js`));
      }
    }
  ...
  mergePluginConfig(plugin) {
    ...
    if (!config) {
      logger.warn(`[avet:loader] pkg.avetPlugin is missing in ${pluginPackage}`);
      return;
    }
    ...
    if (config.name && config.name !== plugin.name) {
      ...
      logger.warn(`[avet:loader] pluginName(${plugin.name}) is different from pluginConfigName(${config.name})`);
    }
